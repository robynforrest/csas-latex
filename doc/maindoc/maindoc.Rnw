...
\fishname\ has been managed on a status-quo basis in recent years, with an annual allocation of 15,000 t for the groundfish trawl fleet since 2006. Before that time, there were no limits on catches or discards of \fishname. There was a notable change in fishing in 2005, when a test fishery was opened out of Port Simpson. The increased catch can be seen in Figure \ref{fig:totalcatch}, especially in the northern areas (5ABCDE) (Panel (c) of Figure \ref{fig:totalcatch}). The areas of high CPUE in Dixon Entrance seen in Figure \ref{fig:cpue} are mainly from this test fishery. Unfortunately, due to rapid proteolysis of the flesh, the fishery was not profitable and a large drop in catch is evident after 2005 (Figure \ref{fig:totalcatch}) when the test fishery ended abruptly. There has also been a recent increase in catch from 2010--2014, which is due to 4 freezer trawlers joining the fleet. With their ability to freeze catch a short time after capture, the freezer trawlers found more profitablilty than the smaller shoreside boats could. The freezer trawlers' priority catch is Pacific Hake (\citet{hake2015}), but when they become scarce on the fishing grounds they have the ability to easily switch gears to bottom trawl to fish for \fishname.\\
...
Reference points based on Maximum Sustainable Yield (MSY), including the female spawning biomass (B\subscr{MSY}) and the annual harvest rate producing MSY (U\subscr{MSY}), were estimated, along with reference based on estimated equilibrium unfished spawning biomass (B\subscr{0}) (Section \ref{sec:FisheryRefPoints}). Harvest decision tables (\ref{tab:Decisions} and \ref{tab:DecisionsSelMat}) were created by projecting the assessment model one year into the future under a range of constant catch levels. For each level of constant harvest, decision tables show the probability that projected stock status in each year will be less than spawning biomass-based reference points, or greater than harvest rate-based reference points (Section \ref{sec:DecisionTables}).\\
...
Commercial fishing data are presented in this document by fishing year which includes the period of January 1, 1996 to December 31, 2014. Landings and discards are shown in Tables \ref{tab:totalcatch} and by area (3CD/5ABCDE) in Table \ref{tab:totalcatchbyarea}.\\

The catch time series data were multiplied by the proportion female for each year (calculated in Appendix \ref{chap:propfemale}), to give a female-only catch series. This series was used in the Reference model and all sensitivity cases presented in this document.\\
...
Extensive ageing was done for this assessment including {\color{red}over 6000} otoliths aged for years spanning 1996--2013, from commercial bottom trawl samples as well as the four research surveys' samples. The samples were aged by the break-and-bake method which involves placing a large number of otoliths in a tray, baking them in a specially-designed oven, then breaking them to perform age reads. During this process, if the person ageing the otoliths finds one which is not baked enough, they will burn the otolith manually to give it the right contrast for age reading. This extra step is what makes this method equivalent to the traditional break-and-burn method in which the ager burns each otolith individually.
...
\begin{enumerate}[noitemsep,nolistsep]
  ...
  \item QCSSS (Figure \ref{fig:agecomps-qcsss}): 2003, 2005, 2011, and 2013.
  \item HSSS (Figure \ref{fig:agecomps-hsss}): 2005, 2007, 2009, 2011, and 2013.
  \item WCVISS (Figure \ref{fig:agecomps-wcviss}): 2004, 2006, 2008, 2010, 2012, and 2014.
\end{enumerate}
...
Variance components of the model were partitioned using an errors in variables approach. The key variance parameter is the inverse of the total variance $\vartheta^2$ (i.e., total precision). The total variance is partitioned into observation and process error components by the model parameter $\rho$, which represents the proportion of the total variance that is due to observation error (\citet{punt1999}, \citet{deriso2007}). The total variance is partitioned into observation errors ($\sigma^2$) and process errors ($\tau^2$) using Eq. \ref{eq:df17}. The parameters $\vartheta$ and $\rho$ were fixed in the current assessment (Table \ref{tab:parampriors}) at values that gave $\sigma$ = $0.2$ and $\tau$ = $0.8$. See Appendix \ref{chap:equations} for details on the treatment of variance in this assessment.\\

\subsubsection{Prior Probability Distributions} \label{sec:priors}
Prior probability distributions for the Reference model are shown in Figure \ref{fig:priors} and Table \ref{tab:parampriors}. Model sensitivities to priors are presented in Sections \ref{sec:priorsteepness}, \ref{sec:priorm}, and \ref{sec:priorsurveyq}..\\
...
Gear selectivity-at-age was estimated for the trawl fishery and the three synoptic surveys (Figure \ref{fig:selex}). Posterior estimates of age-at-50\%-harvest ($sel_{a_1}$) and the standard deviation in the logistic selectivity ogive ($sel_{sd_1}$) are provided in Table \ref{tab:paramest}. The median posterior estimate of age-at-50\%-harvest in the trawl fishery was $9.45$ y. This estimate was further to the right than expected, but was consistent with the available age composition data (Figure \ref{fig:agecomps-trawl}{\color{red} this is the fit figure, do we want the bubble plots with just the comps?}), which indicate very few observations of younger fish, especially in the latter part of the time series. Numerous tests of alternative model configurations did not result in a lower estimate of age-at-50\%-harvest in the trawl fishery. Female \fishname\ are thought to mature at around $5.6$ years of age (Figure \ref{fig:ma}). Therefore, it appears that individuals have several opportunities to spawn before they become vulnerable to the fishery. This in turn resulted in estimates of maximum sustainable harvest rate U\subscr{MSY} approaching 1 (discussed in section \ref{sec:resultsFishRefPoints}), implying that under theoretical equilibrium conditions, all of the vulnerable (i.e., fully-selected) biomass could be harvested because the population could be sustained by younger spawners that are invulnerable to the fishery. We emphasise that this is a theoretical condition subject to the assumptions in the stock assessment model and the data limitations therein. We strongly advise against this as a harvest strategy and suggest that the age-at-50\%-harvest in the trawl fishery is a primary axis of uncertainty in this stock assessment. As discussed above, {\color{red}catches since 2008 have increasingly been represented by the freezer trawler fleet (NEED TO VERIFY THIS)} and age composition data from this fleet were under-represented. Model sensitivity to selectivity in the trawl fishery, in terms of spawning biomass and stock status, is presented in a later section. {\color{red}We recommend as a high priority that future aging requests should include a representative proportion of ages from the freezer trawl fleet for all years since 2005 (again, this might depend on the amount of catch taken by FT fleet)}.\\
%\vspace{10mm}

Selectivities in the three synoptic surveys were also estimated to be quite far to the right (Figure \ref{fig:selex}) with median posterior estimates of age-at-50\%-harvest of $11.53$ y, $9.07$ y and $9.33$ y for the QCSSS, HSSS and WCVIS surveys respectively (Table \ref{tab:paramest}). Standard deviations in the selectivity curves were quite large (Table \ref{tab:paramest}), indicating that selectivity in the surveys was far from knife-edged (Figure \ref{fig:selex}). This was especially true for the QCSSS, where ages showed a less clearly defined pattern than for the other gears, with larger proportions of younger fish (Figure \ref{fig:agecomps-qcsss}). This was likely due to the relatively small sample sizes for this dataset (\ref{fig:agecomps-qcsss}). We recommend that future ageing requests should include ageing of archival otoliths from the QCSSS in order to increase the number of samples and quality of information from this survey.
...
\begin{enumerate}[noitemsep,nolistsep]
  \item The assumed fixed value of variance parameters $\vartheta$ and $\rho$
...
\subsubsection{Alternative values of variance parameters $\vartheta$ and $\rho$}
The Reference model used fixed values variance parameters $\vartheta$ and $\rho$ (Table \ref{tab:paramest}), which resulted in an observation error term ($\sigma= 0.2$) and a process error term ($\tau= 0.8$) (see Appendix \ref{chap:equations}, Eq.\ref{eq:df17}). We tested model sensitivity to these fixed parameters by varying $\vartheta$ to give:
\begin{enumerate}[noitemsep,nolistsep]
...
The Reference model decision table is presented in Table \ref{tab:Decisions}. Alternative 2015 catch levels are presented from 0 t to 30,000 t. Catches are shown in 2,000 t increments from zero to 10,000 t; then 1,000 t increments between 10,000 t and 20,000 t; and then in 2,000 t increments from 22,000 t to 30,000 t. The rows most closely representing the 2014 catch of 10,678 t and the 2014 TAC of 15,000 t are highlighted in bold.
...
cap <- "Posterior (2.5\\textsuperscript{th} percentile, Median, and 97.5\\textsuperscript{th} percentile) and MPD estimates of key parameters from the Reference model. Subscripts on $q$, $sel$, and $selsd$ parameters indicate: 1 = Trawl fishery, 2 = QCSSS, 3 = HSMAS, 4 = HSSS, 5 = WCVISS."
...
\end{center}
\vspace{0mm}
\caption{Trace plots for MCMC output of estimated parameters in the Reference model. The MCMC run had chain length \mcmclength, with a sample taken at every \mcmcnsamp th iteration and the first \mcmcburnin\ samples removed. Parameters $q_k$, $sel_k$, and $selsd_k$ represent gears as follows: $k=1$: commercial trawl; $k=2$: QCSSS: $k=3$; HSMSAS: $k=4$; HSSS, and $k=5$; WCVISS. The HSMAS did not have estimates selectivity, so there are no $sel_3$ or $selsd_3$ parameters.}
\label{fig:traces}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
\vspace{0mm}
\caption{Relative median spawning biomass for the Reference model. The shaded area represents the \mcmcci\ confidence interval.}
\label{fig:depletion}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
\vspace{0mm}
\caption{Estimated recruitment (a) and recruitment deviations (b) for the Reference model. The bars represent the \mcmcci\ confidence interval. The red line is the long-term mean; the green line is the long-term median.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the trawl selectivity parameter. Points and bars represent the estimates of $B_0$ with \mcmcci\ confidence interval.}
\label{fig:sens-sel-biomass}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htp]
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass (a) and recruitment (b) with \mcmcci\ confidence intervals, for models examining sensitivity to the trawl selectivity parameter. Points and bars represent the estimates of $B_0$ with \mcmcci\ confidence interval.}
...
oldPar <- par(no.readonly=TRUE)
layout(matrix(c(1,2),2,1,byrow=FALSE))
plotSelex(plotNum=1,  index=4, sensGroup=g, multiple=TRUE, leg="bottomright", showtitle=FALSE, colors=modelcolors[[g]], linetypes=modellinetypes[[g]], names=modelnames[[g]], indletter=1, add=TRUE)
plotSelex(plotNum=1,  index=5, sensGroup=g, multiple=TRUE, leg=NULL, showtitle=FALSE, colors=modelcolors[[g]], linetypes=modellinetypes[[g]], indletter=2, add=TRUE)
par(oldPar)
@
\end{center}
\vspace{0mm}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the $q_k$ parameter. Points and bars represent the estimates of $B_0$ with \mcmcci\ confidence interval.}
\label{fig:sens-sel-biomass}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}
<<fig.height=5,fig.width=8>>=
# Plot recruitment for sens group 8
g <- 8
plotTS(plotNum=5,  sensGroup=g, ci=ci, burnthin=burnthin, multiple=TRUE, plotMCMC=TRUE, leg="topright", showtitle=FALSE, colors=modelcolors[[g]], linetypes=modellinetypes[[g]], names=modelnames[[g]], opacity=opacity)
@
\end{center}
\vspace{0mm}
\caption{Estimated median recruitment with \mcmcci\ confidence intervals, for models examining sensitivity to the $q_k$ parameter.}
\label{fig:sens-sel-recruitment}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
