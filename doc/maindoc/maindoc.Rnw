...
The catch time series data were multiplied by the proportion female for each year (calculated in Appendix \ref{chap:propfemale}), to give a female-only catch series. This series was used in the Reference Case and all sensitivity cases presented in this document.
...
A female-only, Bayesian statistical catch-at-age model was applied to assess the coastwide stock status of \fishname. The model is based on the Integrated Statistical Catch Age Model (iSCAM) framework, Martell (\citet{martell2011}). Full model details are provided in Appendix \ref{chap:equations}. We define a Reference Case model with fixed and estimated parameters described in Table \ref{tab:parampriors}. A total of {\color{red}78} model parameters were conditionally estimated by the Reference Case model (Table \ref{tab:parampriors}). The model estimated time series of log recruitment anomalies and log fishing mortality rates; and time-invariant values of unfished recruitment, steepness of the Beverton-Holt stock-recruit relationship, natural mortality, average recruitment and logistic selectivity parameters for the commercial fishery and the four surveys. Prior probability distributions for the Reference Case model are shown in Table \ref{tab:parampriors} and Figure \ref{fig:priors} and described in Section \ref{sec:priors}. Model sensitivity to fixed parameters and to assumed prior probability distributions are presented in a later section.\\
\vspace{10mm}
...
Prior probability distributions for the Reference Case model are shown in Figure \ref{fig:priors} and Table \ref{tab:parampriors}. Model sensitivities to priors are presented in Sections \ref{sec:priorsteepness}, \ref{sec:priorm}, and \ref{sec:priorsurveyq}..\\
...
\begin{center}
<<fig.height=8, fig.width=8>>=
plotConvergence(1, 2, burnthin=burnthin, showtitle=FALSE)
@
\end{center}
\vspace{0mm}
\caption{Autocorrelation plots for MCMC output of estimated parameters in the Reference model. The MCMC run had chain length \mcmclength, with a sample taken at every \mcmcnsamp th iteration and the first \mcmcburnin\ samples removed. Here, $m1$ is the natural mortality, $sel1$ represents the selectivity for the trawl fishery; $sel2$ for the QCSSS; $sel4$ for the HSSS; $sel5$ for the WCVISS; $sel3$ is missing because the HSMAS survey did not have age comps estimated in the Reference model. Other parameter names are explained in Figure \ref{fig:priors}}
\label{fig:autocor}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
\vspace{0mm}
\caption{Relative median spawning biomass for \fishname, from an MCMC run with chain length \mcmclength, with a sample taken at every \mcmcnsamp th iteration and the first \mcmcburnin\ samples removed. The shaded area is the \mcmcci\ confidence interval.}
\label{fig:depletion}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{center}
<<fig.height=5,fig.width=8>>=
plotSelex(showtitle=FALSE)
@
\end{center}
\vspace{0mm}
\caption{Estimated selectivity for commercial bottom trawl for \fishname, from an MCMC run with chain length \mcmclength, with a sample taken at every \mcmcnsamp th iteration and the first \mcmcburnin\ samples removed. The bars represent the \mcmcci\ confidence interval. The red line is the long-term mean; the green line is the long-term median.}
\label{fig:selex}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
...
\end{center}
\vspace{0mm}
\caption{Fishing mortality for \fishname, from an MCMC run with chain length \mcmclength, with a sample taken at every \mcmcnsamp th iteration and the first \mcmcburnin\ samples removed. The bars represent the \mcmcci\ confidence interval. The red line is the long-term mean; the green line is the long-term median.}
\label{fig:F}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
\clearpage
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the $\sigma$ parameter.}
...
oldPar <- par(no.readonly=TRUE)
layout(matrix(c(1,2,3,4), 2, 2, byrow=TRUE))
plotTS(plotNum=7, index=2, sensGroup=1, multiple=TRUE, showtitle=FALSE, leg="bottomright", colors=modelcolors[[g]], linetypes=modellinetypes[[g]], names=modelnames[[g]], indletter=1, opacity=opacity, add=TRUE)
plotTS(plotNum=7, index=3, sensGroup=1, multiple=TRUE, showtitle=FALSE, leg=NULL,          colors=modelcolors[[g]], linetypes=modellinetypes[[g]], indletter=2, opacity=opacity, add=TRUE)
plotTS(plotNum=7, index=4, sensGroup=1, multiple=TRUE, showtitle=FALSE, leg=NULL,          colors=modelcolors[[g]], linetypes=modellinetypes[[g]], indletter=3, opacity=opacity, add=TRUE)
plotTS(plotNum=7, index=5, sensGroup=1, multiple=TRUE, showtitle=FALSE, leg=NULL,          colors=modelcolors[[g]], linetypes=modellinetypes[[g]], indletter=4, opacity=opacity, add=TRUE)
par(oldPar)
@
\end{center}
\vspace{0mm}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the $\tau$ parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the steepness parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the standard deviation of the natural mortality  parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the natural mortality parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the catchability parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the trawl selectivity parameter.}
...
\end{center}
\vspace{0mm}
\caption{Estimated median spawning biomass with \mcmcci\ confidence intervals, for models examining sensitivity to the $q_k$ parameter.}
\label{fig:sens-sel-biomass}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%
