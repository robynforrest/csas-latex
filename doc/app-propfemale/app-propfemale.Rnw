...
\section{ALGORITHM}
Pseudocode has been provided in Section \ref{sec:propfemale-pseudocode} to describe the weighting algorithm.
...
the West coast of Vancouver Island, and the combination of Queen Charlotte Sound and Hecate Strait.
...
\subsection{Results}
The proportion of females resulting from this analysis are high, ranging from 0.635 for the 1998 WCVI trawl fishery to 0.909 for the 2008 QCS+HS trawl fishery (Table \ref{tab:propfemale}).
\clearpage
\begin{table}[h]
\centering
...
%<<figBaseSpawnBioMCMC, fig.cap='Spawning biomass for the base model MCMC run.'>>=
%plotTS(scenario=2,plotNum=1,index=2,savefig=FALSE,plotMCMC=TRUE,ci=ci,burnthin=burnthin,ps=ps,leg=NULL,showtitle=FALSE)
%@
