...
\multicolumn{3}{l} {\it {Table \AppLet.1 A list of symbols, constants and description for variables used in iSCAM.  The symbol * after a value indicates that the fixed value was changed for different 2014 assessment scenarios.}} \\ 
 & \\
\hline
\multicolumn{1}{l}{\bf Symbol} & 
...
fecundity and Ricker type recruitment.}% \tabline 
...
%\beq  v_a=(1+\exp(-(\hat{a}-a)/\hat{\gamma}))^{-1}   \label{df5} \eeq \vsd \vsd \vsd
...
%\subsub{MSY-based Reference Points: Constant $M$}
...
State variables in each year are updated using equations \ref{df22}--\ref{df25}, where the spawning biomass is the product of the numbers-at-age and the mature biomass-at-age \eqref{df22}.  The total mortality rate is given by \eqref{df23}, and the total catch (in weight) for each gear is given by \eqref{df24}, assuming that both natural and fishing mortality occur simultaneously throughout the year.  Lingcod catch was not differentiated by sex, so both sexes were combined to calculate a total catch in \eqref{df24}.  The sex-specific numbers-at-age are propagated over time using \eqref{df25}, where members of the plus group (age $A$) are all assumed to have the same total mortality rate.  

Recruitment to age $k$ is assumed to follow a Ricker model for Strait of Gerogia Lingcod \eqref{df26} where the maximum juvenile survival rate ($s_o$) is defined by $s_o=\kappa/\phi_E$.  For the Ricker model, $\beta$ is derived by solving \eqref{df26} for $\beta$ conditional on estimates of $\kappa$ and $R_o$:
\[
\beta = \frac{\ln(\kappa)}{R_o \phi_E}
\]


\subsub{Option for Density-Dependent Natural Mortality}
...
\end{equation}
where $f(B_{t-y})$ is given by \eqref{df26}, and $y$ is the age at recruitment, which is set to 1 for this assessment.  Note that a bias correction term for the lognormal process  errors is included in  \eqref{df26}.
